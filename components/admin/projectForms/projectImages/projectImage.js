...
export default function ProjectImagesForm({
  valueForNextPage,
  mainBackPageValue,
  valueForBack,
}) {
  ...
  useEffect(() => {
    ...
    if (localStorageProjectData) {
      setImage(localStorageProjectData?.Images || "");
      setVideo(localStorageProjectData?.Videos || "");
      setDocuments(localStorageProjectData?.Documents || "");
    }
  }, []);
