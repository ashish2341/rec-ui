...
export default function BasicDetailsForm({ valueForNext, valueForNextPage }) {
  ...
  useEffect(() => {
    const sessionStorageProjectData=JSON.parse(sessionStorage.getItem('EditProjectData'));
    console.log("sessionStorageProjectData from localstorage",sessionStorageProjectData)
    if (sessionStorageProjectData){
      setTitle(sessionStorageProjectData?.Title || "")
      setDescription(sessionStorageProjectData?.Description || "")
      setHighlight(sessionStorageProjectData?.Highlight || "")
    }
  },[]);
  ...
  const SubmitForm = () => {
    ...
    if (title === "") {
      ...
      const updatedProjectData = { ...JSON.parse(sessionStorage.getItem('EditProjectData')), ...basicDetailsData };
      sessionStorage.setItem("EditProjectData",JSON.stringify(updatedProjectData))
